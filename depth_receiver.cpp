...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        std::cout << "Example: " << argv[0] << " 5002" << std::endl;
        return 1;
    }

    int port = std::atoi(argv[1]);

    std::cout << "=== Depth Stream Receiver ===" << std::endl;
    std::cout << "Listening on port: " << port << std::endl;

    try {
        UDPReceiver receiver(port);

        std::cout << "\n=== Receiving Depth Stream ===" << std::endl;
        ...
        while (true) {
            ...
            if (!lastFrame.empty()) {
                ...
                if (age < 1000) {
                    display = lastFrame;
                    cv::putText(display, "DEPTH STREAM - LIVE",
            ...
            cv::imshow("Depth Stream Receiver", display);
