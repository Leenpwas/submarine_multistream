...
// UDP Receiver for 2D Map stream
class UDPReceiver {
private:
    int sockfd;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        ...
        std::cout << "Example: " << argv[0] << " 5003" << std::endl;
        return 1;
    }

    int port = std::atoi(argv[1]);

    std::cout << "=== 2D Map Stream Receiver ===" << std::endl;
    std::cout << "Listening on port: " << port << std::endl;

    try {
        UDPReceiver receiver(port);

        std::cout << "\n=== Receiving 2D Map Stream ===" << std::endl;
        ...
        while (true) {
            ...
            if (!lastFrame.empty()) {
                ...
                if (age < 1000) {
                    display = lastFrame;
                    cv::putText(display, "2D MAP STREAM - LIVE",
            ...
            cv::imshow("2D Map Stream Receiver", display);
